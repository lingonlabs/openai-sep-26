...
export function Presence({ paused, working, monitoring, suggestion, progress, error, send, onPosition }: PresenceProps) {
  ...
  const offer = !working && !paused && suggestion?.id !== handledId ? suggestion : null;
  ...
  return <>
    <button type="button" className={'orb' + (paused || !monitoring ? ' paused' : '')}
      aria-label={working ? 'Ambient is working. Open controls' : paused || !monitoring ? 'Ambient monitoring paused. Open controls' : 'Ambient is watching. Open controls'}
      aria-expanded={!!offer || expanded || working}
    ...
    {(offer || expanded || working || localError || error) && <section className="bubble" aria-label="Ambient assistant" onKeyDown={event => event.stopPropagation()} onKeyUp={event => event.stopPropagation()} onClick={event => event.stopPropagation()}>
      ...
      {busy && <p role="status" className="detail">Sending your choice…</p>}
