...
import { z } from 'zod';
import type { AmbientPreferences, Workspace } from '@ambient/shared';

setTracingDisabled(true);
export const AmbientDecisionSchema = z.object({
  decision: z.enum(['quiet', 'offer', 'question']), summary: z.string().max(5000), reason: z.string().max(1000),
  instructionId: z.string(), tabId: z.number().int(), entityKey: z.string().max(200),
  title: z.string().max(160), detail: z.string().max(1000),
  options: z.array(z.object({ label: z.string().min(1).max(100), prompt: z.string().min(1).max(3000), kind: z.enum(['task', 'dismiss']) })).max(3),
});
...
export const driveAmbient: AmbientDriver = async request => {
  const agent = new Agent({ name: 'Ambient workspace companion', model: request.model,
    outputType: AmbientDecisionSchema, modelSettings: { reasoning: { effort: 'low' } },
...
Be concise, helpful, and restrained. Return quiet when evidence is insufficient.`,
  ...
  const result = await run(agent, JSON.stringify({ workspace: request.workspace, instructions: request.preferences.instructions.filter(i => i.enabled),
    memory: request.summary, recent: request.recent, observations: request.events }), { signal: request.signal, maxTurns: 1 });
