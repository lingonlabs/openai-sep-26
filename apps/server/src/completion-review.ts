...
The previousReviews list shows recovery plans already attempted. Do not stop merely because several attempts have occurred: continue when fresh evidence or a concrete untried supported path makes progress possible. Return blocked if the same bottleneck persists without a genuinely new supported approach. The latest explicit user update takes precedence over stale observations that a PDF is closed or a login is missing; ask the investigator to re-inspect and verify the update rather than asking the user to repeat it. Keep reason and nextStep concise and user-readable.`,
  });
